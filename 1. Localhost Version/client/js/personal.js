document.addEventListener('DOMContentLoaded', function() {
    fetch('http://localhost:8000/getallpersonal')
...
addBtn.onclick = function () {
    ...
    fetch('http://localhost:8000/insertpersonal', {
        headers: {
...
function deleteTableRow(id) {
    fetch('http://localhost:8000/deletepersonal/' + id, {
        method: 'DELETE'
    })
        .then(response => response.json())
        .then(data => {
...
updateBtn.onclick = function() {
    ...
    fetch('http://localhost:8000/updatepersonal/', {
        method: 'PATCH',
        headers: {
            'Content-type' : 'application/json'
        },
