document.addEventListener('DOMContentLoaded', function() {
    fetch('http://localhost:8000/getallschool')
...
addBtn.onclick = function () {
    ...
    fetch('http://localhost:8000/insertschool', {
        headers: {
...
function deleteTableRow(id) {
    fetch('http://localhost:8000/deleteschool/' + id, {
        method: 'DELETE'
    })
        .then(response => response.json())
        .then(data => {
...
updateBtn.onclick = function() {
    ...
    fetch('http://localhost:8000/updateschool/', {
        method: 'PATCH',
        headers: {
            'Content-type' : 'application/json'
        },
